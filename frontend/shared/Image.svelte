<script lang="ts">
	...
	import ModifyUpload from "./ModifyUpload.svelte";
	...
	async function handle_save({ detail }: { detail: string }, initial): Promise<void> {
		if (mode === "mask" && !(source === "webcam" && initial)) {
			value = { back: value?.back || null, mask: detail };
		} else {
			value = { back: detail, mask: null };
		}
		await tick();
	...
	$: {
		if (value === null || (value.back === null && value.mask === null)) {
			static_image = undefined;
		}
	}
	...
	onMount(async () => {
		if (tool === "color-sketch" && value) {
			static_image = value.back;
			await tick();
	...
	{#if source === "upload"}
		...
		>
			{#if (value === null && !static_image) || streaming}
				<slot />
			{:else if tool === "select"}
				<Cropper bind:this={cropper} image={value.back} on:crop={handle_save} />
				<ModifyUpload {i18n} on:clear={(e) => (handle_clear(e), (tool = "editor"))} />
			{:else if tool === "editor"}
				<ModifyUpload
					{i18n}
					on:edit={() => (tool = "select")}
